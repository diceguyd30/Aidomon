...
func test_add_item_stack() -> void:
	self.inventory_manager = auto_free(InventoryManager.new().with_inventory(null))
...
func test_add_item_when_inventory_full() -> void:
	...
	self.inventory_manager = auto_free(InventoryManager.new().with_inventory(inventory_data))
